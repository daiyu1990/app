<template>
	...
	<x-header :left-options="{showBack: true}">报警详情
			<a slot="right">
				<router-link to='/main'>
		...
		<!--     数据传输如何传输的     ？？？-->
		<div class="des_con">
			<p class="alertR btns">塔机编号：{{datas.model}}</p>
			<ul v-for='info in datas.alertR.mes' class="con_ls">
				<li><span class="a_gray">报警描述</span> <span>{{$route.params.tit}}</span></li>
				<li> <span class="a_gray">产权编号</span> <span>{{info.nums}}</span></li>

				<li><span class="a_gray">发生时间</span><span> {{info.time}}</span></li>
