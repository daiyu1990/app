...
class Common_util_pub
{
	 ...
	 * 	作用：生成签名
	 */
	public function getSign($Obj,$wxkey)
	{
		foreach ($Obj as $k => $v)
		{
			$Parameters[$k] = $v;
...
{	
	function __construct($config) 
	{
		...
		$config_info['appid'] = $config['appid'];
		$config_info['mch_id'] = $config['mch_id'];
		$config_info['key'] = $config['key'];
		$this->configinfo = $config_info;
	}
	...
	function createXml()
	{
		try
		{
			...
			if($this->parameters["out_trade_no"] == null) 
			{
		   	...
		   	// $this->parameters["appid"] = WxPayConf_pub::APPID;//公众账号ID
		   	// $this->parameters["mch_id"] = WxPayConf_pub::MCHID;//商户号
		   	$this->parameters['appid'] = $this->configinfo['appid'];
		   	$this->parameters['mch_id'] = $this->configinfo['mch_id'];
		   	$this->parameters["spbill_create_ip"] = $_SERVER['REMOTE_ADDR'];//终端ip	    
		    $this->parameters["nonce_str"] = $this->createNoncestr();//随机字符串
		    $this->parameters["sign"] = $this->getSign($this->parameters,$this->configinfo['key']);//签名
		    return  $this->arrayToXml($this->parameters);
...
class OrderQuery_pub extends Wxpay_client_pub
{
	function __construct($config) 
	{
		...
		//设置curl超时时间
		$this->curl_timeout = WxPayConf_pub::CURL_TIMEOUT;	
		$configinfo['appid'] = $config['appid'];
		$configinfo['mch_id'] = $config['mch_id'];
...
class JsApi_pub extends Common_util_pub
{
	...
	function getOpenid()
	{
		$url = $this->createOauthUrlForOpenid();echo $url.'<br/>';
        //初始化curl
       	$ch = curl_init();
		//设置超时
		...
		curl_close($ch);
		//取出openid
		$data = json_decode($res,true);print_r($data);die;
