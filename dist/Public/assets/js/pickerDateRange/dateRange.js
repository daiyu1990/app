...
function getCookie(name)
{
    ...
    if(getCookie('think_language')=='en-us'){
        var confirm_name = 'Sure';
		...
		gri :[
			'<div id="' + this.calendarId + '" class="gri_dateRangeCalendar">',
				...
				'<div class="gri_dateRangeOptions" '+ (this.mOpts.autoSubmit ? ' style="display:none" ' : '') +'>',
					...
					'</div>',
					'<div>',
						'<input type="button" name="' + this.submitBtn + '" id="' + this.submitBtn + '" value="'+confirm_name+'" />',
						'&nbsp;<a id="' + this.closeBtn + '" href="javascript:;">'+close_name+'</a>',
					'</div>',
				'</div>',
			'</div>'
		],
		ta:[
			'<div id="' + this.calendarId + '" class="ta_calendar ta_calendar2 cf">',
				...
				'<div class="ta_calendar_footer cf" '+ (this.mOpts.autoSubmit ? ' style="display:none" ' : '') +'>',
					...
					'</div>',
					'<div class="frm_btn">',
						'<input class="ta_btn ta_btn_primary" type="button" name="' + this.submitBtn + '" id="' + this.submitBtn + '" value="'+confirm_name+'" />',
						'<input class="ta_btn" type="button" id="' + this.closeBtn + '" value="'+cancel_name+'"/>',
					'</div>',
				'</div>',
			'</div>'
		]
...
pickerDateRange.prototype.fillDate = function(year, month, index) {
	...
	var year_name = getCookie('think_language')=='en-us'?'-':'-';    
    var month_name = getCookie('think_language')=='en-us'?'':'';
    var day_name = getCookie('think_language')=='en-us'?'':'';
	...
	if(isTaTheme){
		...
		if(this.mOpts.magicSelect){
			...
			$(cap).append(yh.join('') +'<span class="joinLine"> '+year_name+' </span>'+mh.join('')+'<span class="joinLine"> '+month_name+' </span>');

		}
		else{
			$(cap).append(year + year_name + (month + 1) + month_name);
		}
	...
	//    $(td).append('<span style="font-size:16px">' + year + '年' + (month + 1) + '月' + '</span>');
		...
		$(td).append(year + year_name + (month + 1) + month_name);
		$(td).attr('colSpan', 7);
		$(td).css('text-align', 'center');
